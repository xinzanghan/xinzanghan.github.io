...
```ts
let sym: symbol = Symbol('xinzanghan')
sym = Symbol('心脏涵~') // OK
sym = '心脏涵~' // Error
```
...
// 任意类型
const anyArray: any[] = ['xinzanghan', 18, {}]
```
...
// 任意类型
const anyArray: Array<any> = ['xinzanghan', 18, {}]
```
...
```ts
const tuple: [string, number] = ['xinzanghan', 18]
...
```ts
type Name = 'xinzanghan' | 'xinzanghan1996' | '心脏涵~'
```
